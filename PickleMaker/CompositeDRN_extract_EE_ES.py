...
def make_feat_compositeDRN(param, shuffle_fraction,x,y,z,E,flag=None,seed=None):
    t0=time()
    x=rescale(x,X_Min,X_Max)
    y=rescale(y,Y_Min,Y_Max)
    z=rescale(z,Z_Min,Z_Max)
    ...
    #else :
    feat = ak.concatenate((x[:,:,None],y[:,:,None],z[:,:,None],E[:,:,None],flag[:,:,None]),-1)
...
class Extract:
    ...
    def read(self, N=None):
        ...
        #feats,class_true = make_feat_compositeDRN(HitsX, HitsY, HitsZ, HitsEn,flg, target,0.5)
        feats,class_true = make_feat_compositeDRN(target,0.5,HitsX, HitsY, HitsZ, HitsEn,flg)
